...
paired_stats.py does all the statistics later -- this script just trains.
...
Results append to results/asrs_results.jsonl, keyed, so reruns are safe.
"""
import argparse
import gzip
import json
...
def main():
    ...
    probe = sp.run(["pgrep", "-f", f"train.py --emb {args.emb} --arch {args.arch}"],
