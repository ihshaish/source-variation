...
import json
import os
DATA = os.environ.get("ASRS_DATA", None)
import re
...
def main():
    with open(os.path.join(DATA or os.path.join(HERE, "data"), "split.json")) as f:
        test_acns = set(json.load(f)["test_acns"])
    sents = []
    with gzip.open(os.path.join(DATA or os.path.join(HERE, "data"), "task_aircraft.jsonl.gz"), "rt") as f:
        for line in f:
            r = json.loads(line)
    ...
    out = os.path.join(DATA or os.path.join(HERE, "data"), "fasttext_asrs_train_200d.bin")
    model.save(out)
