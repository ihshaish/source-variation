# Feeds Table 2 (seed-averaged differences and joint record-resampling intervals) and Supplementary Tables S11 and S13. Arithmetic on the prediction files written by records_train.py, records_meanpool.py, control_roberta.py and nhtsa_task.py; nothing trains.
...
recs=[json.loads(l) for l in gzip.open(f'{HERE}/records_task.jsonl.gz','rt')]
...
yN=np.load(f'{NHTSA}/nhtsa_preds_summary_bilstm_s0.npz')['y']; nN=len(yN)
F={f:[pred(np.load(f'{NHTSA}/nhtsa_preds_{f}_bilstm_s{s}.npz')) for s in (0,1,2)] for f in ('summary','conseq','remedy')}
...
json.dump(out,open(f'{RESULTS}/views/seedavg.json','w'),indent=1)
print('DONE')
