...
python3 records_train.py --record narr|syn --emb glove200|w2vview [--smoke]"""
...
def main():
    ...
    ap.add_argument('--record',required=True,choices=['narr','syn'])
    ...
    if a.record=='narr': vocab|=set(t for r in recs if r['r2toks'] for t in r['r2toks'])
    idx,matrix=build_matrix(a.emb,a.record,vocab)
    cap=CAPS[a.record]
    ...
    rp=os.path.join(HERE,'records_results.jsonl')
    if os.path.exists(rp):
        done={json.loads(l)['key'] for l in open(rp)}
    for seed in ((0,) if a.smoke else (0,1,2)):
        key=f"{tag}final_{a.record}_{a.emb}_s{seed}"
        ...
        np.savez(os.path.join(HERE,f"{tag}preds_{a.record}_{a.emb}_s{seed}.npz"),probs=pte,y=yte,acns=acns[is_test])
        out={"key":key,"view":a.record,"emb":a.emb,"seed":seed,"test_macro_f1":round(float(f1),4),
