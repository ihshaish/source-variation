...
Emits control_roberta_declared.json."""
import json,os
import numpy as np
...
for s in (0,1,2):
    ...
    bl={v:pred(np.load(os.path.join(NHTSA,f'nhtsa_preds_{v}_bilstm_s{s}.npz'))) for v in ('summary','remedy')}
...
tf=np.load(os.path.join(NHTSA,'nhtsa_preds_summary_tfidf_s0.npz'))
for s in (0,1,2):
