...
json.dump(gstats,open(os.path.join(OUT,"nhtsaND_grouping.json"),"w"),indent=1)
...
json.dump({"classes":classes,"n":len(data),"n_test":int(te.sum())},open(os.path.join(OUT,'nhtsaND_task_meta.json'),'w'))
...
for vk,field in VIEWS.items():
    ...
    Xtr=v.fit_transform([r[field] or '' for r,m in zip(data,~te) if m])
    ...
    np.savez(os.path.join(OUT,f'nhtsaND_preds_{vk}_tfidf_s0.npz'),pred=pr,probs=prob,y=y[te])
...
for vk,field in VIEWS.items():
    toks=[TOKEN_RE.findall((r[field] or '').lower())[:CAP] for r in data]
    ...
    for seed in (0,1,2):
        ...
        np.savez(os.path.join(OUT,f'nhtsaND_preds_{vk}_bilstm_s{seed}.npz'),pred=pr,y=y[te])
        res.append({"key":f"nhtsa_{vk}_bilstm_s{seed}","f1":round(float(mf1(y[te],pr)),4)})
        log(res[-1])
json.dump(res,open(os.path.join(OUT,'nhtsaND_results.json'),'w'),indent=1)
...
for arch,seeds in (('tfidf',(0,)),('bilstm',(0,1,2))):
    for a,b in pairs:
        for s in seeds:
            da=np.load(os.path.join(OUT,f'nhtsaND_preds_{a}_{arch}_s{s}.npz'))
            db=np.load(os.path.join(OUT,f'nhtsaND_preds_{b}_{arch}_s{s}.npz'))
...
json.dump(cons,open(os.path.join(OUT,'nhtsaND_contrasts.json'),'w'),indent=1)
