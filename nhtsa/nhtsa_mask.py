"""Same as nhtsa_task.py, except that every word appearing in a class name
(the sixteen top-level component names) is removed from the text before the
TF-IDF features and the word2vec tokens are built, so a classifier cannot
read the label off the text. Task build, split, seeds and models are
otherwise identical. Writes nhtsa_mask_meta.json, nhtsa_mask_results.json,
nhtsa_mask_contrasts.json and one nhtsa_mask_preds_*.npz per run.
Run: python3 nhtsa_mask.py"""
import json
import os
import re
import numpy as np
...
MASK = set()
for name in classes:
    MASK.update(re.findall(r'[a-z]+', name.lower()))
print("mask tokens:", sorted(MASK))


def strip_mask(text):
    return ' '.join(word for word in re.split(r'(\W+)', text or '') if word.lower() not in MASK)
...
json.dump({"classes": classes, "n": len(data), "n_test": int(is_test.sum())}, open(os.path.join(HERE, 'nhtsa_mask_meta.json'), 'w'))
...
for field_key, field in FIELDS.items():
    ...
    Xtr = vectorizer.fit_transform([strip_mask(record[field]) for record, m in zip(data, ~is_test) if m])
    Xte = vectorizer.transform([strip_mask(record[field]) for record, m in zip(data, is_test) if m])
    clf = LogisticRegression(max_iter=2000).fit(Xtr, y[~is_test])
    pred = clf.predict(Xte)
    probs = clf.predict_proba(Xte)
    np.savez(os.path.join(HERE, f'nhtsa_mask_preds_{field_key}_tfidf_s0.npz'), pred=pred, probs=probs, y=y[is_test])
...
for field_key, field in FIELDS.items():
    token_lists = [[w for w in TOKEN_RE.findall((record[field] or '').lower()) if w not in MASK][:CAP] for record in data]
    ...
    for seed in (0, 1, 2):
        ...
        np.savez(os.path.join(HERE, f'nhtsa_mask_preds_{field_key}_bilstm_s{seed}.npz'), pred=pred, y=y[is_test])
        results.append({"key": f"nhtsa_{field_key}_bilstm_s{seed}", "f1": round(float(macro_f1(y[is_test], pred)), 4)})
        log(results[-1])
json.dump(results, open(os.path.join(HERE, 'nhtsa_mask_results.json'), 'w'), indent=1)
...
for arch, seeds in (('tfidf', (0,)), ('bilstm', (0, 1, 2))):
    for field_a, field_b in pairs:
        for seed in seeds:
            preds_a = np.load(os.path.join(HERE, f'nhtsa_mask_preds_{field_a}_{arch}_s{seed}.npz'))
            preds_b = np.load(os.path.join(HERE, f'nhtsa_mask_preds_{field_b}_{arch}_s{seed}.npz'))
...
json.dump(contrasts, open(os.path.join(HERE, 'nhtsa_mask_contrasts.json'), 'w'), indent=1)
log("NHTSA MASK DONE")
