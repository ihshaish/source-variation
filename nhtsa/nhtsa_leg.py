...
groups={}
for r in data: groups.setdefault(r['Summary'].strip().lower()[:400],[]).append(r)
...
for vk,field in VIEWS.items():
    ...
    for seed in (0,1,2):
        ...
        log(res[-1])
json.dump(res,open(os.path.join(HERE,'nhtsa_results.json'),'w'),indent=1)
...
json.dump(cons,open(os.path.join(HERE,'nhtsa_contrasts.json'),'w'),indent=1)
log("LEG B DONE")
