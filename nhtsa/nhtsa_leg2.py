...
json.dump({"classes":classes,"n":len(data),"n_test":int(te.sum())},open(os.path.join(HERE,'nhtsa2_task_meta.json'),'w'))
...
for vk,field in VIEWS.items():
    ...
    np.savez(os.path.join(HERE,f'nhtsa2_preds_{vk}_tfidf_s0.npz'),pred=pr,probs=prob,y=y[te])
...
for vk,field in VIEWS.items():
    ...
    for seed in (0,1,2):
        ...
        np.savez(os.path.join(HERE,f'nhtsa2_preds_{vk}_bilstm_s{seed}.npz'),pred=pr,y=y[te])
        res.append({"key":f"nhtsa_{vk}_bilstm_s{seed}","f1":round(float(mf1(y[te],pr)),4)})
        log(res[-1])
json.dump(res,open(os.path.join(HERE,'nhtsa2_results.json'),'w'),indent=1)
...
for arch,seeds in (('tfidf',(0,)),('bilstm',(0,1,2))):
    for a,b in pairs:
        for s in seeds:
            da=np.load(os.path.join(HERE,f'nhtsa2_preds_{a}_{arch}_s{s}.npz'))
            db=np.load(os.path.join(HERE,f'nhtsa2_preds_{b}_{arch}_s{s}.npz'))
...
json.dump(cons,open(os.path.join(HERE,'nhtsa2_contrasts.json'),'w'),indent=1)
log("LEG B DONE")
